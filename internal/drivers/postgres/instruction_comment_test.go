...
func TestAnnotateInstructions(t *testing.T) {
	t.Run("NoInstruction", func(t *testing.T) {
		require.Empty(t, driversshared.AnnotateInstructions(nil))
	})
	...
	t.Run("OneCommentForAGroupOfOneObject", func(t *testing.T) {
		dropColumn := &PostgresAlterTableInstruction{
			...
			Actions: []driversshared.AlterTableAction{
				&driversshared.SQLDropColumnAction{
					ColumnName: "age",
				},
			},
		}
		...
		require.Equal(t, []driversshared.Instruction{
			...
			dropColumn,
			dropConstraint,
		}, driversshared.AnnotateInstructions([]driversshared.Instruction{dropColumn, dropConstraint}))
	})

	t.Run("CommentOfTheDriver", func(t *testing.T) {
		note := commentInstruction(`The table "logs" holds no primary key`)
		insert := &driversshared.SQLInsertInstruction{
			TableName: "users",
		}
		...
		require.Equal(t, []driversshared.Instruction{
			...
			insert,
		}, driversshared.AnnotateInstructions([]driversshared.Instruction{note, insert}))
	})
}
