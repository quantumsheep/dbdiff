...
func (d *SQLiteDriver) Diff(ctx context.Context) ([]driversshared.Instruction, error) {
	...
	if holdsTableRecreation(instructions) {
		instructions = append([]driversshared.Instruction{&SQLitePragmaForeignKeysInstruction{}}, instructions...)
...
func holdsTableRecreation(instructions []driversshared.Instruction) bool {
	return lo.SomeBy(instructions, func(instruction driversshared.Instruction) bool {
		_, isRecreation := instruction.(*SQLiteTableRecreationInstruction)

		return isRecreation
	})
}

func (d *SQLiteDriver) DiffTables(ctx context.Context) ([]driversshared.Instruction, error) {
	...
	additions, removals, err := driversshared.DiffByKey(targetTables, sourceTables, driversshared.DiffRules[*SQLiteTable]{
		Key: func(table *SQLiteTable) string {
			return table.Name
		},
		Create: func(table *SQLiteTable) []driversshared.Instruction {
			return table.Instructions()
		},
		Change: func(target *SQLiteTable, source *SQLiteTable) ([]driversshared.Instruction, error) {
			return target.DiffTable(source)
		},
		Drop: func(table *SQLiteTable) []driversshared.Instruction {
			return []driversshared.Instruction{&driversshared.SQLDropTableInstruction{
				Name: table.Name,
			}}
		},
	})
...
func (d *SQLiteDriver) DiffViews(ctx context.Context) ([]driversshared.Instruction, error) {
	...
	additions, removals, err := driversshared.DiffByKey(targetViews, sourceViews, driversshared.DiffRules[*SQLiteView]{
		Key: func(view *SQLiteView) string {
			return view.Name
		},
		Create: func(view *SQLiteView) []driversshared.Instruction {
			return view.Instructions()
		},
		Change: func(target *SQLiteView, source *SQLiteView) ([]driversshared.Instruction, error) {
			return target.Diff(source)
		},
		Drop: func(view *SQLiteView) []driversshared.Instruction {
			return []driversshared.Instruction{&driversshared.SQLDropViewInstruction{
				Name: view.Name,
			}}
		},
	})
...
func (d *SQLiteDriver) GetTableColumns(ctx context.Context, db *sql.DB, tableName string) ([]*SQLiteColumn, error) {
	...
	rows, err := db.QueryContext(ctx, "PRAGMA table_xinfo("+driversshared.QuoteIdentifier(tableName)+");")
	if err != nil {
		return nil, err
	}
...
func (d *SQLiteDriver) GetTablePrimaryKey(ctx context.Context, db *sql.DB, tableName string) ([]string, error) {
	rows, err := db.QueryContext(ctx, "PRAGMA table_info("+driversshared.QuoteIdentifier(tableName)+");")
	if err != nil {
		return nil, err
	}
...
func (d *SQLiteDriver) GetTableUniqueKeys(ctx context.Context, db *sql.DB, tableName string) ([][]string, error) {
	rows, err := db.QueryContext(ctx, "PRAGMA index_list("+driversshared.QuoteIdentifier(tableName)+");")
	if err != nil {
		return nil, err
	}
...
func (d *SQLiteDriver) GetIndexColumnNames(ctx context.Context, db *sql.DB, indexName string) ([]string, error) {
	rows, err := db.QueryContext(ctx, "PRAGMA index_info("+driversshared.QuoteIdentifier(indexName)+");")
	if err != nil {
		return nil, err
	}
...
func (d *SQLiteDriver) GetIndexKeys(ctx context.Context, db *sql.DB, indexName string, definitionKeys []string) ([]string, error) {
	...
	for rows.Next() {
		...
		if name.Valid {
			...
			keys = append(keys, driversshared.QuoteIdentifier(name.String)+modifiers)
...
func (d *SQLiteDriver) GetTableForeignKeys(ctx context.Context, db *sql.DB, tableName string) ([]*SQLiteForeignKey, error) {
	rows, err := db.QueryContext(ctx, "PRAGMA foreign_key_list("+driversshared.QuoteIdentifier(tableName)+");")
	if err != nil {
		return nil, err
	}
